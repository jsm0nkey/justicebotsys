...
const youtube = new YouTube("AIzaSyDy9JzY5ak505plgKOMUW-xbn5DxTPAMPY"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyDy9JzY5ak505plgKOMUW-xbn5DxTPAMPY"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@489047547235794944>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (message.content.startsWith("ناحو")) {
               if(!message.channel.guild) return message.reply('** This command only for servers**');
         
  if(!message.guild.member(message.author).hasPermission("BAN_MEMBERS")) return message.reply("**محتاجة راجل**");
...
message.channel.send(`**:white_check_mark: ${user.tag} بيقلك مرا ناحو🤣 ! :airplane: **  `)
...
client.on("message", message => {
  if (message.content.startsWith("بره")) {
    if (message.author.bot) return;
    if (!message.channel.guild)
      return message.reply("** This command only for servers**");

    if (!message.guild.member(message.author).hasPermission("KICK_MEMBERS"))
      return message.reply("**محتاجة راجل**");
    ...
    const kickembed = new Discord.RichEmbed()
      .setAuthor(`مع السلامة!`, user.displayAvatarURL)
      .setColor("RANDOM")
      .setTimestamp()
...
client.on("message", message => {
  ...
  if (command ===   "ششش") {
    ...
    if (message.guild.member(user).roles.has(muteRole.id)) {
      return message
        .reply("**:white_check_mark: .. بتاكد بس 😂 **")
        .catch(console.error);
    } else {
      message.guild
        .member(user)
...
client.on("message", message => {
  ...
  if (message.content.startsWith( "تعالي")) {
    if (message.member.hasPermission("MOVE_MEMBERS")) {
      if (message.mentions.users.size === 0) {
        return message.channel.send("``Use : " +  "تعالي@User``");
      }
...
\`${prefix}embed\` : لعمل رساله امبيد
\`${prefix}امسح\` : لمسح الشات 
\`${prefix}ناحو\` : لحظر شخص من السيرفر
\`${prefix}بره\` : لطرد شخص من السيرفر
\`${prefix}افتح\` : لفتح الشات
\`${prefix}اقفل\` : لقفل الشات 
\`${prefix}ششش\` : لإسكات شخص
\`${prefix}اتكلم\` : لـ فك إسكات شخص
\`${prefix}new\` : فتح التكت
\`${prefix}closet\` : لحذف روم التكت
\`${prefix}say\` : البوت يكرر كلامك
\`${prefix}تعالي\` : لسحب الشخص الى روومك
